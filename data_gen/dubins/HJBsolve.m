...
%                 if ~(((ismember(k,init_S)) || (ismember(k,init_S+K-1))) && (ismember(j,init_Y)) && (ismember(i,init_X)))
                % alternative: skip if value function is 0, equivalently
                if ~(u0(i,j,k) == 0)
...
%                     %v = 1, w = 1
%                     u1 = u0(i,j,k) + dt*(1+(A/dx)*(u0(i+a,j,k)-u0(i,j,k))...
%                         +(B/dy)*(u0(i,j+b,k)-u0(i,j,k))...
%                         +(W/ds)*(u0(i,j,kp1)-u0(i,j,k)));
%                     %v = 1, w = -1
%                     u2 = u0(i,j,k) + dt*(1+(A/dx)*(u0(i+a,j,k)-u0(i,j,k))...
%                         +(B/dy)*(u0(i,j+b,k)-u0(i,j,k))...
%                         +(W/ds)*(u0(i,j,k-1)-u0(i,j,k)));
%                     %v = 1, w = 0
%                     u3 = u0(i,j,k) + dt*(1+(A/dx)*(u0(i+a,j,k)-u0(i,j,k))...
%                         +(B/dy)*(u0(i,j+b,k)-u0(i,j,k)));
