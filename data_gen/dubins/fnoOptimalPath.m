function [p] = fnoOptimalPath(grid,uN,W,d,x0,y0,s0,xf,yf,sf)
% function [p] = optimalPath(grid,u,W,d,x0,y0,s0,xf,yf,sf)
% [X,Y,S] = ndgrid(grid.x(2:end-1),grid.y(2:end-1),grid.s);
[X,Y,S] = ndgrid(grid.x, grid.y, grid.s);
size(X)
size(Y)
size(S)
size(uN)
uN = griddedInterpolant(X,Y,S,uN);
